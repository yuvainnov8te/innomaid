...
function cal()
{ var total=0;
 $('input[ name="package_price[]"]').each(function(){
if($(this).val() != "")
...
<h3 style='margin-left:10px;'>Edit @if($_REQUEST['mode'] == 'newtransfer') {{'New/Transfer'}} @else {{'Replacement'}} @endif Service</h3>
<hr/>
<div class="panel-body" style="">    
  {!! Form::model($service,  array('route' => array('sentinel.service.update', $service->id))) !!}
               <Input type="hidden" name="mode" value="{{$_REQUEST['mode']}}">
                ...
                <div class="row">
                    ...
                    <label for="Name of FDW">Fee According to Package?</label>
                     ...
                     @if($packageList->count() === 0 && $standardpackage->count() === 0)
                       <div class="col-xs-5 {{ ($errors->has('name')) ? 'error' : '' }}">
                     ...
                     <div class="col-xs-5 {{ ($errors->has('name')) ? 'error' : '' }}">
                      ...
                      <input name="according_package" type="checkbox"  checked='checked' id="package_fee">
                      @else
         ...
         <div class="row" style="max-width: 100%;"  id="p_fee">
          ...
          @else 
           ...
           <input type="hidden" name="package_id[]" value="<?php echo $plan->package_id; ?>">
           ...
           <td style="padding:0px 0px 0px 9px; ">
                     <input type="text" name='package_price[]' value="<?php echo $plan->package_price; ?>" class='form-control package' onchange="cal()" placeholder='S$'>
            </td>
            </tr>
             @endforeach 
              @foreach($standardpackage as $plan)
           <input type="hidden" name="package_id[]" value="<?php echo $plan->id; ?>">
           <tr><td style="padding:0px;"> <input type="text" name='package[]' value="<?php echo $plan->package_name; ?>" class= 'form-control' readonly> </td>
           <td style="padding:0px 0px 0px 9px; ">
                     <input type="text" name='package_price[]' class='form-control package' onchange="cal()" placeholder='S$'>
                     </td> 
            </tr> 
             @endforeach
          @endif
		...
		<div class="row">
                    ...
                    <label for="Name of FDW">Selected by default. </label>
                     ...
                     <div class="col-xs-5">
			@if($service->default_selected=='Y')
                      <input name="default_selected" type="checkbox" value='Y' checked='checked'>
			@else
